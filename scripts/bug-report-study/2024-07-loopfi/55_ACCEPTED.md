# #55: `Flashlender.sol#flashLoan()` should use `mintProfit()` to mint fees. The current implemetation may lead to locked up WETH in PoolV3.
Labels: ['bug', '3 (High Risk)', 'primary issue', 'satisfactory', 'selected for report', 'sponsor confirmed', 'sufficient quality report', ':robot:_primary', ':robot:_189_group', 'H-13']
Accepted: True

# Lines of code

https://github.com/code-423n4/2024-07-loopfi/blob/main/src/Flashlender.sol#L106
...
When using flashlender to create loans, there is small amount of fee. The fees are sent to PoolV3 and is spread out to the lpETH stakers.
...
The impact of this issue is that the WETH amount in PoolV3 would be larger than expectedLiquidity, and since lpETH:WETH is always 1:1, when users want to withdraw the WETH, it may revert due to underflow when updating expectedLiquidity. An example is:
...
Note that CDPVault also uses `mintProfit()` to send the extra liquidation penalty as profit to PoolV3. https://github.com/code-423n4/2024-07-loopfi/blob/main/src/CDPVault.sol#L569
