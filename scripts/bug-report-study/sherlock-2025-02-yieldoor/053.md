...
1. The `strategy` must be deployed for a pool with an odd tickSpacing (1 or 5).

### External Pre-conditions

 1. the spot tick needs to satisfy the following condition:


$$
\text{spotTick} = k \times \text{tickSpacing} + \left(\frac{\text{tickSpacing}}{2}\right) + 1
$$
...
1. `Rebalancer` rebalances the `Strategy`.
