...
In `GatewayTransferNative.sol`, the [`onCall()`](https://github.com/sherlock-audit/2025-05-dodo-cross-chain-dex/blob/main/omni-chain-contracts/contracts/GatewayTransferNative.sol?plain=1#L357) function is not marked `payable`. However, it invokes [`_doMixSwap()`](https://github.com/sherlock-audit/2025-05-dodo-cross-chain-dex/blob/main/omni-chain-contracts/contracts/GatewayTransferNative.sol?plain=1#L425), which internally calls:
```solidity
IDODORouteProxy(DODORouteProxy).mixSwap{value: msg.value}(...)
```
Since `onCall()` is not **payable**, any ETH sent to it will cause the transaction to revert, or more dangerously, `msg.value` will be zero, leading to failed swaps or incorrect behavior.

### Internal Pre-conditions

1) A user sends ETH as part of the cross-chain message to trigger a swap using `onCall()`.

2) The target swap operation requires ETH to be passed into `_doMixSwap()`.



### External Pre-conditions

1) The external gateway sends a call to `onCall()` with native ETH.

2) The DODO `_doMixSwap` expects a non-zero `msg.value`.
