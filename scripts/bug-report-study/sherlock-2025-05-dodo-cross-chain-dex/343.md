...
# Lack of gas limit setter will lock protocol functionality post-deployment
...
In [`GatewayCrossChain.sol:35`](https://github.com/sherlock-audit/2025-05-dodo-cross-chain-dex/blob/main/omni-chain-contracts/contracts/GatewayCrossChain.sol#L35) there is no function defined to modify the `gasLimit` variable after it has been set in the `initialize` function.

### Internal Pre-conditions

1. Owner calls `initialize()` to set `gasLimit` to a fixed value  
2. No further call is possible to update `gasLimit` due to missing setter


### External Pre-conditions

1. Gas costs on the target chain increase beyond the initially configured `gasLimit`  
2. Cross-chain transactions begin to fail due to insufficient gas forwarding
