...
2. In the payload, they set `params.fromToken = T₍val₎_` and `params.fromTokenAmount` to the contract’s balance of *T₍val₎*.
...
contract GatewayCrossChainTest is BaseTest {
    function test_Exploit_DrainValuableToken() public {
        ...
        // The variables feePercent, platformFeesForTx, and amountForSwapAfterPlatformFee are now calculated
        // above, before creating swapDataZ_malicious. These are used by the test for payload construction
        // and for assertions. The contract itself will use the `cheapTokenDepositAmount` from `onCall`
        // to perform its internal fee calculations.
        ...
        // Adjusting expectation for cheapToken received by attacker:
        // The DODO mock `dodoRouteProxyZ.setPrice(address(token1Z), address(token2Z), 2e18);` means 1 token1Z -> 2 token2Z.
        // The malicious swap sells `amountForSwapAfterPlatformFee` of `valuableToken` (token1Z).
        // So, attacker should receive `amountForSwapAfterPlatformFee * 2` of `cheapToken` (token2Z).
        assertEq(
        ...
        uint256 dodoProxyInitialToken1ZBalance =
            token1Z.balanceOf(address(dodoRouteProxyZ)) - amountForSwapAfterPlatformFee; // Calculate what it was before this swap
        assertEq(
...
* **Invariant check** before calling `_doMixSwap`:
