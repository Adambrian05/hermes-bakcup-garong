...
1. **Relayer / user** must invoke `GatewayCrossChain.onCall` with
   `decoded.dstChainId == SOLANA_EDDY` so that the contract executes

   ```solidity
   AccountEncoder.encodeInput(AccountEncoder.decompressAccounts(decoded.accounts), …)
   ```
2. **Relayer / user** sets at least one account entry where
   *byte N* (the flag) = `0x00` **and** *byte N + 1 … N + 31* contain a non-zero byte.
